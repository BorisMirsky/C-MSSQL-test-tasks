-- Переименовал таблицу Routes в RoutesTable. 
-- Добавил в неё новый столбец Route_Id - Id маршрута. 
...
-- У нас 5 машин с разной грузоподъёмностью, отсюда разный остаток груза после рабочего дня.
...
-------------------------------------------------------------------------------------
-- 1.3. Найти самую быструю машину или доказать, что по представленным данным это невозможно сделать

-- 1. Все машины приезжают на склад под загрузку в одинаковое время, поэтому нет необходимости
-- считать разницу между DepartureTime и ArrivalTime, достаточно сравнить DepartureTime.
-- 2. У нас 5 машин с разной грузоподъёмностью, отсюда разное время загрузки. Поэтому данные о скорости
-- передвижения по маршруту коррелируют с грузоподъёмностью.
-- 4. Здесь также достаточно одного запроса, который покажет условную неэффективность по критерию 'скорость'для маршрута и машин.
--select top(5) Route_Id, Car_Id, max(cast(DepartureTime as time)) as MaxTime
--from RoutesTable
--group by Route_Id, Car_Id
